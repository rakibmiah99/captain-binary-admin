...
@section('content')
    ...
    <script>
        ...
        deleteData('/testimonial/delete/:id')


        //Show Data
        $('#table-contents').on('click', '.table-show-btn', function (){
            let id = $(this).attr('data-id');
            requestData('/testimonial/show/'+id, {
                method: 'GET'
            }).then(function (response){
        ...
        $('#table-contents').on('click', '.table-edit-btn', function (){
            let id = $(this).attr('data-id');
            setFormUrl('/testimonial/update/'+id, 'edit');
            modalLoaderON()
            requestData('/testimonial/show/'+id, {
                method: 'GET'
            }).then(function (response){
                let data = response.data.data;
                setFormData(data.name, data.designation, data.comments);
                modalLoaderOFF();
            }).catch(function (error){
        ...
        $('.create-btn').on('click', () => {
            modalLoaderOFF();
            setFormUrl('/testimonial/store')
        })


        function setFormData(name = "", designation = "", comments = ""){
            let createForm = $('.create-form');
            createForm.find('input[name=name]').val(name)
            createForm.find('input[name=designation]').val(designation)
            createForm.find('textarea[name=comments]').val(comments)
        }
        ...
        function appendRow(selector,item, index = 1){
            $(selector).append(`
                 <tr>
                    ...
                    <td><img class="rounded" style="height: 70px" src="${item.img}" alt="${item.name}"/></td>
                    <td>${item.designation}</td>
                    <td>${printWithLimit(item.comments, 15)}</td>
